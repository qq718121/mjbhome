...
import qs from 'qs'
import { getDAesString, desc } from './des'
import { url } from './urlLocation'
import store from './store';

export default function ({ urls = '', data = {}, params = {}, headers = {}, methods = 'get', responseType = '', des = false, types = 0, token = '' }) {
     ...
     // axios.defaults.baseURL = url.httpRequestse;111
     ...
     switch (methods) {
          ...
          case 'post':
               if (des) {
                    data = qs.stringify({ d: desc(JSON.stringify(s), keys) }) || {}
               } else {
                    data = qs.stringify({ d: JSON.stringify(s) }) || {}
               }
               break;
          default:
               break;
