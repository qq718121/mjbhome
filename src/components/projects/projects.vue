<template>
          <div class='projects' v-if="dataErr">
                    ...
                    <div class='projectRate'>
                              <div v-for='(val,index) in projectsData.designArr' :key="index">
                    ...
                    <div class='projectHr'>
                              <i></i>
                              <img src='../../../static/gray.svg'>
                              <span>主要板块评测</span><img src='../../../static/gray.svg'>
                              <i></i>
                    </div>
                    <div class='allAppraisal'>
                              ...
                              <div class='elevatorDoor' v-for='(value,index) in projectsData.project.types' :key="index">
                                        <div>
                    ...
                    <div class='park'>
                              ...
                              <div class='parkDoorText' v-for="(value,index) in projectsData.park.types">
                                        <span>{{value.type}}</span>
                                        <span>{{value.name == '8' ? '有' : value.name == '9' ? '无' : value.name}}</span>
                                        <p v-html='value.messages'></p>
                              </div>
                              <div class='parkDoor' v-for='(value,index) in projectsData.park.typeLv' :key="index">
                                        <div>
                                                  <span :class="$class_name_ftn.projects_classNames(value.lv)">{{$class_name_ftn.storeNam(value.lv)}}</span>
                                                  <p>{{value.name}}</p>
                                        </div>
                    ...
                    <div class='foot' v-for='(value,index) in projectsData.houseType' :key="index">
                              <div class='footTop'>
                                        ...
                                        <div class="footIndex">
                                                  ...
                                                  <i></i>
                                                  <div class='psText'>
                                                            <p v-for='(key,index) in value.houseTypeLv' :key="index">
                                                                      <span>{{key.name}}</span>
                                        ...
                                        <div class='footDoorText' v-for='(j,index) in value.get' v-if="j.name" :key="index">
                                                  <p>赠送面积</p>
                                                  <p>{{j.name}}</p>
                                                  <p class='footGet' v-for='(tes,index) in j.messages' :key="index">{{tes}}</p>
                                        </div>
                              </div>
                    </div>
          </div>
...
export default {
          ...
          methods: {
                    ...
                    getData() {
                              ...
                              let url = "planDesign/" + this.$route.params.id;
                              let options = {
                                        urls: url,
                                        methods: 'get',
                                        des: true,
                              ...
                              this.$http(options).then(res => {
                                        ...
                                        this.footTopImg = res.houseType.img;
                                        this.houseTypeName = res.houseType.houseTypeName;
                                        this.psTextArr = res.houseType.houseTypeLv;
                              }).catch(err => {
                                        this.set_hint('数据错误');
                              })
                              /*END*/
...
.footTop {
        ...
        background: #ffffff;
        box-shadow: 0 0.02rem 0.12rem 0 rgba(0, 0, 0, 0.09),
                0 1px 0.04rem 0 rgba(0, 0, 0, 0.15);
        border-radius: 0.08rem;
}
