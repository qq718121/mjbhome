<template>
  <div class="m-comment">
    <nav class="m-nav">
      ...
      <div class="m-nav-btn" @click="f_load('1','animated bounceIn')"><span>{{load_text}}</span></div>
    </nav>
    <div class="g-banner">
    ...
    <div class="g-banner-pro">
      <div class="g-tit">
        <div class="g-user">
          <div class="u-user-img"><img :src="com_data.avatar ?com_data.avatar :de_use_img "
                                       alt=""></div>
      ...
      <div class="g-inner">
        ...
        <div class="g-sign"><span v-for="item in com_data.tagList">{{item.tagName}}</span></div>
      </div>
      <div class="g-load" @click="f_load('0','animated bounceIn')">
        <div class="u-btn"><span>在APP中查看更多楼盘照片</span></div>
      </div>
    </div>
    <div class="g-comment">
      <div class="u-tit"><span>业主评论</span><span>({{reply_data.length}})</span></div>
      <div class="g-com-ine" v-for="item in reply_data">
        ...
        <div class="g-com-tet" style="width: 6.22rem;">
          <div>
            <div class="g-com-ine-top"><span class="u-user-nam">{{item.userName}}</span></div>
            <p style="margin-top: 0.2rem" class="u-tet">{{item.content ? item.content : '暂无数据'}}</p>
            <div style="margin-top: 0.09rem"><span class="u-tim">{{item.date}}</span></div>
          </div>
          <div class="g-com-reply" v-if="item.replyNum > 0">
            <div>
              ...
              <div style="margin-top: 0.2rem;"><p class="u-tet">
                {{item.replyContent ? item.replyContent : '暂无数据'}}</p></div>
            </div>
            <div style="margin-top: 0.2rem" v-if="item.replyNum - 1 > 0" @click="f_load('0','animated bounceIn')"><span
              class="u-reply-btn">查看{{item.replyNum - 1}}条回复</span>
            </div>
          </div>
        </div>
      </div>
