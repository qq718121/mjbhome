  ...
  <div class="configure">
    <div class="bannerWap">
      ...
      <div class="repeatWap">
        <div>
          <p>
            <span
              style="font-family: PingFangSC-Thin;font-size: 0.8rem;color: #5880F3;line-height: 0.8rem;text-align:center;margin-top:0.2rem;font-weight:200">{{data.buildScore}}</span>
            <span>周边配套评级</span>
          </p>
          <p>
    ...
    <div class="scoreWap">
      <div class="item" v-if="index<3" v-for="(item,index) in data.buildScoreDetailList" :key="index">
        <span class="span1" :class="{'activeiconbac1':item.ratio==1,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==1">优+</span>
        <span class="span1" :class="{'activeiconbac1':item.ratio==2,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==2">优</span>
        <span class="span1" :class="{'activeiconbac2':item.ratio==3,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==3">良</span>
        <span class="span1" :class="{'activeiconbac3':item.ratio==4,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==4">中</span>
        <span class="span1" :class="{'activeiconbac4':item.ratio==5,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==5">差</span>
        <span class="span1" :class="{'activeiconbac5':item.ratio==6,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==6">无</span>
        <span class="span1" :class="{'activeiconbac6':item.ratio==7,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==7">轻微</span>
        <span class="span1" :class="{'activeiconbac6':item.ratio==8,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==8">一般</span>
        <span class="span1" :class="{'activeiconbac6':item.ratio==9,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==9">较严重</span>
        <span class="span1" :class="{'activeiconbac6':item.ratio==10,'activeiconbac7':item.ratio==6}"
              v-if="item.ratio==10">严重</span>
    ...
    <div class="itemWap1" v-for="(item,index) in data.buildScoreDetailList" :key="index">
      ...
      <div class="bigWap" v-if="item.id==1" :class="{'activebac1':item.id==1}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san1.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san1.svg" type="image/svg+xml"></embed>
      </div>
      <div class="bigWap" v-if="item.id==2" :class="{'activebac2':item.id==2}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san2.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san2.svg" type="image/svg+xml"></embed>
      </div>
      <div class="bigWap" v-if="item.id==3" :class="{'activebac3':item.id==3}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san3.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san3.svg" type="image/svg+xml"></embed>
      </div>
      <div class="bigWap" v-if="item.id==4" :class="{'activebac4':item.id==4}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san4.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san4.svg" type="image/svg+xml"></embed>
      </div>
      <div class="bigWap" v-if="item.id==5" :class="{'activebac5':item.id==5}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san5.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san5.svg" type="image/svg+xml"></embed>
      </div>
      <div class="bigWap" v-if="item.id==6" :class="{'activebac6':item.id==6}">
        <embed style="position:absolute;left:0;top:-4px;width:.15rem" src="static/img/san6.svg"
               type="image/svg+xml"></embed>
        <p>{{item.simpleEvaluation}}</p>
        <embed style="position:absolute;right:0;top:-4px;width:.15rem;transform:rotateY(180deg)"
               src="static/img/san6.svg" type="image/svg+xml"></embed>
      </div>
...
<style scoped>
  ...
  .configure .bannerWap .repeatWap .p2, .configure .bannerWap .repeatWap .p3 {
    font-size: 12px;
    color: #9E9E9E;
  }
