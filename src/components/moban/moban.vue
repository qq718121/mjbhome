<template>
     <div class="moban">
          <div>
               ...
               <div class="moban_" @touchmove.prevent v-if='this.$store.state.school_class'>
                    <div :class="['centers',this.$store.state.school_class]" v-if="this.$store.state.motai_num == '0' ">
                         <div class='loadText'>
                              <p>{{loadTexts}}</p>
                              <p>更多楼盘质量数据尽在鹰眼鉴房</p>
                              <a @click='loadHref'>{{loadHrefs}}</a>
                              <img :src=this.$url.moban_pic_load_title>
                              <i @click="cloce" class='el-icon-close' v-if="this.$store.state.is_close"></i>
                         </div>
                    </div>
                    <div :class="['web_obshare',this.$store.state.school_class]" v-if="this.$store.state.motai_num == '1' ">
                         <img src="/static/fenxiang@3x.png" alt=""></div>
                    <div class="click_1" @click="cloce" v-if="this.$store.state.motai_num == '1' || this.$store.state.motai_num == '2'"></div>
                    ...
                    <div :class="['login-centers',this.$store.state.school_class]" style="position: relative" v-if="this.$store.state.motai_num == '3' && !isCodeSucc">
                         <div class="g-down-ipt">
                              <el-input v-model="code" placeholder="输入验证码" type="number" @input="inputCode" clearable></el-input>
                         </div>
                    ...
                    <div :class="['login-centers',this.$store.state.school_class]" v-if="this.$store.state.motai_num == '3' && isCodeSucc">
                         <div class="g-suc">
                              <div class="u-suc-tit">
                                   <img src="http://oxrgdeqd8.bkt.clouddn.com/icon_chenggong@3x.png" alt=""></div>
                         </div>
                         <div class="g-suc-ine">
                              <p>成功加入鹰眼鉴房</p>
                              <p>{{pathNum}}后自动跳转</p>
...
export default {
     ...
     methods: {
          ...
          ...mapState({ dat_com: "dat_com", data_comment: "data_comment" }),
          //关闭
          cloce() {
          ...
          loadHandler() {
               ...
               var options = {
                    ...
                    methods: 'post',
                    data: {
                         userPhone: JSON.parse(datas).userPhone,
                         type: '1'
                    },
                    des: true,
                    types: 3
          ...
          //提交验证码
          submitCode() {
               var url = "user/quickLogin";
               var options = {
                    urls: url,
                    methods: 'post',
                    data: {
                         userPhone: window.sessionStorage.getItem("phones"),
                         smsCode: this.code
                    },
                    des: true,
                    types: 3
          ...
          pathNumTime() {
               ...
               let id = setInterval(() => {
                    ...
                    if (num == 0) {
                         clearInterval(id);
                         this.cloce();
                         this.$router.push({ path: "/logindown" });
                    }
               }, 1000);
          },
          codeTime() {
          ...
          openApp() {
               var iphoneSchema = "",
                    iphoneDownUrl =
                         "https://itunes.apple.com/cn/app/%E9%B9%B0%E7%9C%BC%E9%89%B4%E6%88%BF/id129840873" +
                         "6?mt=8",
                    androidSchema = "",
                    androidDownUrl =
                         "http://owvcynyz1.bkt.clouddn.com/homehawkeye1.4.3.1.apk";
               if (this.dat_com() == "0") {
               ...
               if (Obshare.is_Weixin()) {
                    //          window.location = 'http://tweb.homehawkeye.com/appDown.html' + url_n
                    // + url_m;
               } else {
                    //非微信浏览器
