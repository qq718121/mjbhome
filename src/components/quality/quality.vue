<template>
          <div class='quality' v-if="dataErr">
                    ...
                    <div class='qualityHr'>
                              ...
                              <span>施工质量评级</span><img src='../../../static/gray.svg'>
                              <i></i>
                    </div>
                    ...
                    <div class='qualityOne' v-for='(key,index) in this.arr'>
                              ...
                              <div class='qualityOnePlan'>
                                        ...
                                        <el-progress :text-inside="true" :stroke-width="18" :percentage="parseFloat(key.sum)" status="success" style='width:6rem;margin:auto;'></el-progress>
...
export default {
          ...
          methods: {
                    ...
                    getData() {
                              ...
                              /*START*/
                              let url = "quality/" + this.$route.params.id;
                              let options = {
                                        urls: url,
                                        methods: 'get',
                                        des: true,
                              ...
                              this.$http(options).then(res => {
                                        this.dataErr = true;
                                        this.topImg = res.img ? res.img : this.$url.error_img;
                                        this.houseName = res.houseName;
                                        this.data = res.date;
                                        this.plan = res.plan;
                                        this.lv = res.lv;
                                        this.qualityFootArr = res.siteImg;
                                        this.allEvaluate = res.allEvaluate;
                                        for (let i in res.qualityLvArr) {
                                                  this.qualityLvArr[i].lv = res.qualityLvArr[i].lv;
                                                  continue;
                                        }
                                        for (let i in res.qualityPlanArr) {
                                                  for (let j in res.qualityPlanArr[i].list) {
                                                            this.qualityPlanArr[i].list[j].plan = res.qualityPlanArr[i].list[j].plan * 1;
                                                  }
                                                  this.arr[i].sum = res.qualityPlanArr[i].sum;
                                                  this.arr[i].text = res.qualityPlanArr[i].desc ? res.qualityPlanArr[i].desc : '暂无数据！';
                                                  continue;
                                        }
                              }).catch(err => {
                                        this.set_hint('数据错误');
