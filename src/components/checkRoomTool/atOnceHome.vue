<template>
     <div class="atOnceHome">
          ...
          <div class="g-atOnceHome" v-show="prvLoad">
               <div class="g-atOnceHome-list" v-for="(val,index) in atOnceHome_list" :key="index" @click="f_atOnceHome_listHandler(val.path,val.id,val.name)">
     ...
     methods: {
          ...
          handler() {
               ...
               var d = { token: t, body: { arr: [{ commonUse: [] }] } },
                    datas = null,
                    url = "buildingCamera/roomCheckingTool";
               for (let i = 0; i < 6; i++) {
                    let obj = { id: i + 7 };
