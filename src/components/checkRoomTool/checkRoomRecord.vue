<template>
     <div class="checkRoomRecord">
          ...
          <span style="color:#5e5e5e;display: inline-block;width: 100%;text-align: center;line-height: 0.5rem;margin-top: 0.5rem" v-if="testOverData.length <=0">暂无数据</span>
          <div class="g-checkRoomRecord-list" v-for="(item, index) in testOverData" :key="index">
               <div class="g-checkRoomRecord-list-tit" @click="f_go_checkRoomOver(item)">
                    ...
                    <i style="margin-right:0" class="u-g-checkRoomRecord-icon"><img src="/static/tool/housingesta_icon_xiangqing@3x.png" alt=""></i>
               </div>
...
export default {
     ...
     methods: {
          ...
          getOver() {
               ...
               var url = "buildingCamera/roomCheckResultList";
               var options = {
                    urls: url,
                    methods: 'post',
                    token: t,
