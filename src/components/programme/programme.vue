<template>
  <div class='programme'>
    ...
    <div class='programmeHr'>
      ...
      style='font-family: PingFangSC-Light;font-size: 16px;color: #42464D;'>规划落实评级</span><img
    ...
    <div class='programmeHr'>
      <i></i><img src='../../../static/gray.svg'><span
      style='font-family: PingFangSC-Light;font-size: 16px;color: #42464D;'>总体评价</span><img
      src='../../../static/gray.svg'><i></i>
    </div>
    <div class='programmeOveralls' v-html="programmeData.allEvaluate"></div>
    <div class='programmeHr'>
      <i></i><img src='../../../static/gray.svg'><span
      style='font-family: PingFangSC-Light;font-size: 16px;color: #42464D;'>规划落实评测</span><img
      src='../../../static/gray.svg'><i></i>
    </div>
    <div class='programmeOne' v-for='(key,index) in arr'>
      <div class='programmeOneTop'>
        <div class='programmeOneTitle'><span><img :src='key.img'></span><span>{{key.programmePlanData.title}}</span>
        </div>
        <div :class="key.className"><i></i><i></i>
          <p>{{key.programmePlanData.text}}</p></div>
      </div>
      ...
      <div class='programmeOnePlan' v-for="tex in key.programmePlanData.programmePlanText">
        <p><span>{{tex.title}}</span><span>{{(tex.sum + "%")}}</span></p>
        <el-progress :text-inside="true" :stroke-width="18" :percentage="parseFloat(tex.sum)" status="success"
                     style='width:6rem;margin:auto;'></el-progress>
        <p v-for="value in tex.part"><span>{{value.name}}<i
          :class='gbClass(value.bg)'>{{gbTitle(value.bg)}}</i></span><span
          :class="judgeClass(value.qualified)">{{gbJudge(value.qualified)}}</span></p>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      this.$Axios.get(this.$url.httpRequest + 'planPracticable/' + this.$route.params.id).then(function (res) {
        let data = JSON.parse(this_.$getDAesString(res, "yhgt!d%sd*aw%dSDSFSsa#mng~dsq"));
        this_.programmeData = data.response.data;
        for (let i in this_.programmeData.programmeLv) {
          this_.arr[i].programmePlanData = this_.programmeData.programmeLv[i];
//          console.log(this_.arr[i].programmePlanData);
        }
        this_.topImg = data.img ? data.img : '../../../static/pig_imageholder@3x.png';
